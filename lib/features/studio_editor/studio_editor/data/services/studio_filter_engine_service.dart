  ...
  for (var y = 0; y < height; y++) {
    for (var x = 0; x < width; x++) {
      ...
      if (cfg.isSepia) {
        ...
        final shiftedCb =
            sig.meanCb + ((cb - targetMeanCb) * (sig.stdCb / (targetStdCb + 1e-6)));
        final shiftedCr =
            sig.meanCr + ((cr - targetMeanCr) * (sig.stdCr / (targetStdCr + 1e-6)));
      ...
      var finalR = _clamp255(originalR * protect + rgb[0] * (1.0 - protect));
      var finalG = _clamp255(originalG * protect + rgb[1] * (1.0 - protect));
      var finalB = _clamp255(originalB * protect + rgb[2] * (1.0 - protect));
...
Future<Uint8List> runStudioFilterEngine(Map<String, dynamic> params) async {
  ...
  final signature = analyzeUnifiedStyle(refImage);
